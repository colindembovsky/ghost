...
if [ -z $CDN ]; then
  echo "Please set CDN environment variable!"
    ...
    x=10
    trap exit TERM; while [ $x -gt 0 ]; do sleep 30s & wait $!; echo "Reloading config (30s)"; nginx -s reload; x=$(( $x - 1)); done & nginx

    # loop infinitely, reloading the ngix config every 6 hours
    trap exit TERM; while :; do sleep 6h & wait $!; echo "Reloading config (6h)"; nginx -s reload; done & nginx
fi