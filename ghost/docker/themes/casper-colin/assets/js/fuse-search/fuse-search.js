...
class FuseSearch {
    constructor(args) {
        ...
        const defaults = {
            ...
            button: '',
            defaultValue: '',
            template: function(result, score) {
                ...
                drawResults: function(results) {
                    for (let key in results){
                        if (key < results.length) {
                            document.querySelectorAll(this.results)[0].appendChild(
                                this.createElementFromHTML(
                                    this.template(results[key].item, results[key].score)
                                )
                            );
                        };
                    }
    ...
    displayResults(data){
        ...
        inputValue = this.on.processSearchTerm(inputValue);

        const fuse = new Fuse(data, this.options);
        const beforeResults = fuse.search(inputValue);
        
        const results = this.on.processResults(beforeResults);
        
        this.on.drawResults(results);
